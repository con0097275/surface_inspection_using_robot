...
class TensorImages:
    ...
    def plot_input_pred_resmap(self, index, group, save_dir=None):
        ...
        if save_dir is not None:
            ...
            #edit
            if (os.path.isdir(os.path.join(save_dir.replace("/","\\\\"), plot_name).rsplit("\\",1)[0])):
                pass
            else:
                os.makedirs(os.path.join(save_dir.replace("/","\\\\"), plot_name).rsplit("\\",1)[0])
            fig.savefig(os.path.join(save_dir.replace("/","\\\\"), plot_name))
